<?php
	...
	if ($_SERVER["REQUEST_METHOD"] === "POST") {
		...
		$stmt = $conexao->prepare("UPDATE desenvolvedoras SET nome = :campo2, pais = :campo3, campo5 = :campo5 WHERE id = :id");
		$stmt->execute([
			':nome' => $campo2,
			':pais' => $campo3
			':campo5' => $campo5,
	...
	if (isset($_GET['id'])) {
		...
		$stmt = $conexao->prepare("SELECT * FROM desenvolvedoras WHERE id = :id");
		$stmt->execute([':id' => $id]);
		$registro = $stmt->fetch();
